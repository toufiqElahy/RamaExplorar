defmodule BlockScoutWeb.API.V2.AddressControllerTest do
  ...
  describe "/addresses/{address_hash}/token-transfers" do
    ...
    test "check token type filters", %{conn: conn} do
      ...
      erc_20_token = insert(:token, type: "RAMA-20")
      ...
      erc_721_token = insert(:token, type: "RAMA-721")
      ...
      erc_1155_token = insert(:token, type: "RAMA-1155")
    ...
    test "type and direction filters at the same time", %{conn: conn} do
      ...
      filter = %{"type" => "RAMA-721", "filter" => "from"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
      assert response["items"] == []
      assert response["next_page_params"] == nil

      filter = %{"type" => "RAMA-721", "filter" => "to"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
      ...
      filter = %{"type" => "RAMA-721,RAMA-20", "filter" => "to"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
      ...
      filter = %{"type" => "RAMA-721,RAMA-20", "filter" => "from"}
      request = get(conn, "/api/v2/addresses/#{address.hash}/token-transfers", filter)
      assert response = json_response(request, 200)
    ...
    test "check that same token_ids within batch squashes", %{conn: conn} do
      ...
      token = insert(:token, type: "RAMA-1155")
    ...
    test "check that pagination works fine with 1155 batches #1 (large batch) + check filters", %{conn: conn} do
      ...
      filter = %{"type" => "RAMA-1155", "filter" => "to"}
      ...
      filter = %{"type" => "RAMA-1155", "filter" => "from"}
  ...
  describe "/addresses/{address_hash}/tokens" do
    ...
    test "get tokens", %{conn: conn} do
      ...
      ctbs_erc_20 =
        for _ <- 0..50 do
          insert(:address_current_token_balance_with_token_id, address: address, token_type: "RAMA-20", token_id: nil)
          |> Repo.preload([:token])
        end
      ...
      ctbs_erc_721 =
        for _ <- 0..50 do
          insert(:address_current_token_balance_with_token_id, address: address, token_type: "RAMA-721", token_id: nil)
          |> Repo.preload([:token])
        end
        |> Enum.sort_by(fn x -> x.value end, :asc)
  ...
  def check_total(%Token{type: nft}, json, token_transfer) when nft in ["RAMA-1155"] do
    json["token_id"] in Enum.map(token_transfer.token_ids, fn x -> to_string(x) end) and
      json["value"] == to_string(token_transfer.amount)
  end

  def check_total(%Token{type: nft}, json, token_transfer) when nft in ["RAMA-721"] do
  ...
  def check_total(%Token{type: nft}, json, _token_transfer) when nft in ["RAMA-721", "RAMA-1155"] and is_list(json) do
    false
  end
